...
import { Type, Keyword } from './dbdl.grammar.terms';
import { type SQLDialect, SQLNamespace } from './index';
...
class CompletionLevel {
  ...
  addCompletions(completions: readonly (Completion | string)[]) {
    for (let option of completions)
      this.addCompletion(
        typeof option == 'string' ? nameCompletion(option, 'property', this.idQuote) : option
      );
  }
